...
const Reports = () => {
  ...
  const exportToCSV = () => {
    ...
    if (reportType === 'inventory') {
      // Headers
      csvContent = 'Product ID,Name,SKU,Category,Price,Quantity,Supplier,Reorder Level,Status\n';
      ...
      reportData.forEach(item => {
        ...
        csvContent += `${date},"${item.product?.name}",${item.product?.sku},${item.type},${item.quantity},"${item.performedBy?.name}","${item.notes || ''}"\n`;
      });
    }
  ...
  const exportToPDF = () => {
    ...
    // Title
    doc.setFontSize(18);
    doc.text(`${reportType === 'inventory' ? 'Inventory' : 'Transaction'} Report`, 14, 20);
                      ...
                      <tr key={item._id} className="border-b border-border">
                        ...
                        <td className="py-3 px-2">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            ...
                            item.quantity <= item.reorderLevel ? 'bg-amber-100 text-amber-700' :
